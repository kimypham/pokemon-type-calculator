...
function PokemonSearch() {
    ...
    const getTypeData = async (pokemonType: string): Promise<IType> => {
        return await axios.get("https://pokeapi.co/api/v2/type/" + pokemonType)
            .then((response) => {
    ...
    const calculateDamage = (typeData: IType, currentTakeDamageArray: IDamage, currentGiveDamageArray: IDamage): void => {
    ...
    useEffect(() => {
        const currentTakeDamageArray = JSON.parse(JSON.stringify(takeDamageArray));
        const currentGiveDamageArray = JSON.parse(JSON.stringify(giveDamageArray));

        pokemonData?.types.forEach(async (pokemon) => {
            const typeData = await getTypeData(pokemon.type.name);
            if (typeData)
                calculateDamage(typeData, currentTakeDamageArray, currentGiveDamageArray);
        ...
        let takeEffectiveness: Effectiveness = {}
        Object.keys(takeDamageArray).forEach((type: string) => {
            const castedType: keyof IDamage = type as keyof IDamage

            const takeEffectivenessKey = takeDamageArray[castedType] as keyof typeof takeEffectiveness


            if (takeEffectiveness[takeEffectivenessKey]) {
                takeEffectiveness?[takeEffectivenessKey].push(type)
            } else {
                takeEffectiveness[takeEffectivenessKey] = [castedType]
            }

            console.log(type)
            console.log(takeDamageArray[type as keyof IDamage])
        })
        console.log("takeEffectiveness", takeEffectiveness)
